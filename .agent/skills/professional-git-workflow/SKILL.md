...
2.  **Verify via PR Preview**: Do not rely on "it works on my machine" or manual `firebase hosting:channel:deploy` URLs (which can be unreliable). The **PR Preview URL** generated by GitHub Actions is the single source of truth.
